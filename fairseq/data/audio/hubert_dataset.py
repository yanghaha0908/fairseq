...
def load_audio(manifest_path, max_keep, min_keep):
    ...
    with open(manifest_path) as f:
        root = f.readline().strip()  #'/mnt/lustre/sjtu/shared/data/asr/rawdata/LibriSpeech/dev-clean'
        for ind, line in enumerate(f):   #ind:0  line:'1272/128104/1272-128104-0000.flac	93680'
            ...
            sz = int(items[1]) #sz：93680
            if min_keep is not None and sz < min_keep:  #min_keep:32000, max_keep=None
                n_short += 1
            elif max_keep is not None and sz > max_keep:
                n_long += 1
            else:
                names.append(items[0])
                inds.append(ind)  #index 0
                sizes.append(sz)
    tot = ind + 1   #train:281241 #valid:tot:2703  #train:names[list:280531] #valid:names[list:2666] 那几个也是
    logger.info(
        (
...
def verify_label_lengths(
    ...
    for i, ind in enumerate(inds):
        dur_from_audio = audio_sizes[i] / audio_rate  #5.855
...
class HubertDataset(FairseqDataset):
    ...
    def collater(self, samples):
        ...
        audios = [s["source"] for s in samples]  #list:5 5个Tensor
        audio_sizes = [len(s) for s in audios]  #[475760, 446720, 434240, 429359, 419280]
        if self.pad_audio:
            audio_size = min(max(audio_sizes), self.max_sample_size)
        else: #
            audio_size = min(min(audio_sizes), self.max_sample_size)  #audio_size:250000   #self.max_sample_size:250000 在yaml里有
    ...
    def collater_audio(self, audios, audio_size):
        collated_audios = audios[0].new_zeros(len(audios), audio_size) #(5,250000)  全0
        padding_mask = (
        ...
        return collated_audios, padding_mask, audio_starts  #collated_audios（5，250000）padding_mask（5，250000） 全false # audio_starts:[3223, 188605, 9448, 117415, 153946]  裁的那段的起点

    def collater_frm_label(self, targets, audio_size, audio_starts, label_rate, pad):
        assert label_rate > 0
        s2f = label_rate / self.sample_rate #0.003125
        frm_starts = [int(round(s * s2f)) for s in audio_starts]  #[10, 589, 30, 367, 481] [3223, 188605, 9448, 117415, 153946]
        frm_size = int(round(audio_size * s2f)) #781
        ...
        targets = data_utils.collate_tokens(targets, pad_idx=pad, left_pad=False)   #没step这个！
