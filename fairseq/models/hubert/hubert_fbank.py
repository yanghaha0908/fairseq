...
@register_model("hubert_fbank", dataclass=HubertConfig)
class HubertfbankModel(BaseFairseqModel):
    def __init__(
        ...
        # new!  sr_ctc 中的 为了fbank
        from fairseq.models.hubert.convsubsampler import Conv1dSubsampler, Pooling1DSubsampler, SuperFrame        
        #### downsample layer config
        self.encoder_embed_dim=512
        self.dropout=0.1
        
        self.conv_kernel_sizes: str = "5,5"
        # conv_kernel_sizes: str = "5"
        self.conv_channels: int = 1024
        self.input_feat_per_channel: int = 80
        self.input_channels = 1

        self.subsample = Conv1dSubsampler(
            self.input_feat_per_channel * self.input_channels,
            self.conv_channels,
        ...
        stats_npz_path = "/data/ygr/global_cmvn.npy"  #原始路径 /mnt/lustre/sjtu/home/xc915/data/Gigaspeech/gigaspeech-s-rawwav/global_cmvn.npy “两个是一样的”
    ...
    def extract_fbank_features(self, source):  # torch.Size([5, 60960])
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(source.size(0)):
                #_waveform = source[batch_idx][:src_lengths[batch_idx]] 因为音频没有padding 都是真实长度 
                _waveform = source[batch_idx]  #torch.Size([60960])
                ...
                # if self.encoder.training is True:   pretrain的时候不用specaug
                #     features = self.specaug_transform(features)

                fbank_features.append(features)  # torch.Size([5, 1469, 80])
                fbank_lengths.append(feat_len)  #tensor([1469, 1469, 1469, 1469, 1469], device='cuda:0', dtype=torch.int32)
    ...
    def forward_features(self, source: torch.Tensor) -> torch.Tensor:
        if self.feature_grad_mult > 0:  #pretrain是0.1
            #features = self.feature_extractor(source)  #（8，512，477） //修改这里！
            features, encoder_out_lengths = self.extract_fbank_features(source)   #torch.Size([368, 5, 512]), tensor([368, 368, 368, 368, 368], device='cuda:0')
            if self.feature_grad_mult != 1.0:
                features = GradMultiply.apply(features, self.feature_grad_mult)  #torch.Size([368, 5, 512])
        else:
            with torch.no_grad():
                #features = self.feature_extractor(source) #这  #（8，512，686）
                features,encoder_out_lengths = self.extract_fbank_features(source) 
        return features
    ...
    def forward(
        ...
        # t1=datetime.now() 
        # #features = self.forward_features(source)   #就是过 ConvFeatureExtractionModel （8，512，477） 原来是 B*C*T  #source (8,152960)
        # features = self.forward_features(source)   # features: T * B * C 
        # #print(features.shape)  #torch.Size([368, 5, 512])
        # features=features.transpose(0, 1)  
        # features=features.transpose(1, 2)  #为了和原来一样 搞成 B*C*T
        # #print(features.shape)   #(5,512,368)
        # t2=datetime.now() 
        # print("time delay:%f"%((t2-t1).seconds) )
        ...
        features = features.transpose(1, 2)  #（8，477，512） #需要 B*T*C  #fbank: torch.Size([5, 368, 512])
