...
class TransformerEncoder(nn.Module):
    ...
    def extract_features(
        ...
        if padding_mask is not None:
            x = index_put(x, padding_mask, 0)   #应该没有padding_mask  应该是没变
        #x 本来是 （5,735,768）
        x_conv = self.pos_conv(x.transpose(1, 2))  #（5,768,735）  #5一个Conv1d
        x_conv = x_conv.transpose(1, 2)  #（5,735,768）
        x = x + x_conv #（5,735,768）

        if not self.layer_norm_first:
            x = self.layer_norm(x)  #（5,735,768）

        # pad to the sequence length dimension
        x, pad_length = pad_to_multiple(
            x, self.required_seq_len_multiple, dim=-2, value=0  #我这个数是没变，我怀疑跟奇偶有关   （5,736,768）pad_length=1
        )
        if pad_length > 0 and padding_mask is None:
            ...
            padding_mask, _ = pad_to_multiple(
                padding_mask, self.required_seq_len_multiple, dim=-1, value=True  #把padding mask也补了 (5,736)
            )
        x = F.dropout(x, p=self.dropout, training=self.training)  #（19，602，768）

        # B x T x C -> T x B x C
        x = x.transpose(0, 1)  #(736,5,768)

        layer_results = []
        r = None
        for i, layer in enumerate(self.layers):#i=0-11
            dropout_probability = np.random.random() if self.layerdrop > 0 else 1
            if not self.training or (dropout_probability > self.layerdrop):   #not self.training false  #self.layerdrop=0.05
                x, (z, lr) = layer(
                    x, self_attn_padding_mask=padding_mask, need_weights=False
                )   #x,lr都是（602，19，768）   #再细看下这里
                if i >= min_layer: #0
                    layer_results.append((x, z, lr))   #有问题
            if i == tgt_layer:  #tgt_layer None
                r = x
                break
        ...
        if pad_length > 0:
            ...
            layer_results = [undo_pad(*u) for u in layer_results]    #每一个layer result 是 return x, (attn, layer_result) attn=None  layer_result 就是没加x0的
...
class TransformerSentenceEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.layer_norm_first:
            ...
            residual = x
            x = self.activation_fn(self.fc1(x))  #fc1:Linear(in_features=768, out_features=3072, bias=True)  self.activation_fn  gelu
            x = self.dropout2(x)
            x = self.fc2(x)    #fc2:Linear(in_features=3072, out_features=768, bias=True)
        ...
        return x, (attn, layer_result)  #attn=None  layer_result 没加残差
